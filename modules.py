...
class DrugGraphNet(nn.Module):
    def __init__(self, in_feats, dim_embedding=128, padding=True, hidden_feats=None, activation=None):
        ...
        self.gcn_model = GCN(in_feats=dim_embedding, hidden_feats=hidden_feats, activation=activation)
        self.gat_model = GAT(in_feats=dim_embedding, hidden_feats=hidden_feats, num_heads=[4])
        self.init_transform = nn.Linear(in_feats, dim_embedding, bias=False)
    ...
    def forward(self, batch_graph):
        ...
        node_feats = self.gcn_model(batch_graph, node_feats)

        # node_feats = self.gat_model(batch_graph, node_feats)
        node_feats = self.gat_model(batch_graph, node_feats)
...
class InvertedResidualBlock(nn.Module):
    def __init__(self, inp, oup, expand_ratio):
        ...
        self.bottleneckBlock = nn.Sequential(
            ...
            # dw
            nn.Conv1d(hidden_dim, hidden_dim, 3, groups=hidden_dim, padding=1, bias=False),
            nn.BatchNorm1d(hidden_dim),
            nn.ReLU6(inplace=True),
            # pw-linear
...
class ProteinIR(nn.Module):
    def __init__(self, embedding_dim, num_filters, kernel_size, padding=True):
        super(ProteinIR, self).__init__()
        if padding:
        ...
        self.in_ch = in_ch[-1]
        kernels = kernel_size
        self.res_block1 = InvertedResidualBlock(in_ch[0], in_ch[1], kernels[0])
        self.res_block2 = InvertedResidualBlock(in_ch[1], in_ch[2], kernels[1])
